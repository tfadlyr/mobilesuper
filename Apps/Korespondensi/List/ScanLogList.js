...
function ScanLogList() {
  ...
  async function getScanLog(page) {
    setIsLoading(true);
    try {
      let response = await getHTTP(nde_api.scanlog.replace("{$page}", page));
      let data = initData(list, response.data);
      setList(data);
      setIsLoading(false);
    } catch (error) {
      handlerError(error, "Warning!", "Scan Log List not working");
      setIsLoading(false);
    }
  }
      ...
      <View style={{ marginBottom: 16 }}>
        {item.children.map((data) => (
          <CardList
            ...
            tipe="scanlog"
            onPress={() => {
              navigation.navigate("ScanLogDetail", {
                id: data.id,
                title: "Scan Log\nDetail",
              });
            }}
