...
function InternalSatkerList({ route }) {
  ...
  async function getAgendaInInternal(page) {
    setIsLoading(true);
    try {
      let response;
      if (unread) {
    ...
    try {
      ...
      if (error?.response?.status == 401 || error?.status == 401) {
        ...
        handlerError(
          error,
          "Peringatan!",
          "Pencarian Internal Satker tidak berfungsi"
        );
      }
      ...
      <View style={{ marginBottom: 16 }}>
        {item.children.map((data) => (
          <CardList
            key={data.id}
            data={data}
            tipe="agendaininternal"
            onPress={() => {
              navigation.navigate("IncomingDetail", {
                id: data.id,
                title: "Detail Internal Satker",
                tipe: "agendaininternal",
              });
            }}
          />
...
export default InternalSatkerList;
