    ...
    try {
      ...
      if (unread) {
        response = await getHTTP(
          nde_api.agendadispounread.replace("{$page}", page)
        );
      } else {
        response = await getHTTP(nde_api.agendadispo.replace("{$page}", page));
      }
  ...
  const filter = async (page) => {
    ...
    try {
      if (
        ...
        let url;
        if (unread) {
          url = nde_api.agendadispounread;
        } else {
          url = nde_api.agendadispo;
        }
